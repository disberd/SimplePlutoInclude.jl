...
function module_and_names(m::Module, extract_names_func)
    ...
    target_module = m
    if length(nms) === 1 
        only_item = invokelatest(getproperty, m, first(nms))
...
function expr_from_module(m::Module)
    invokelatest(isdefined, m, :plutoinclude_helpers) || error("The expr_from_module function only works with modules generated by @plutoinclude")
    (;
        extracted_names,
        target_ex,
        compiled_path
    ) = invokelatest(getproperty, m, :plutoinclude_helpers)
